# Discard Button
extends Button

signal discard_requested


func _ready() -> void:
	pressed.connect(_on_pressed)
	_connect_discard_listeners()

func _on_pressed() -> void:
	discard_requested.emit()


func _connect_discard_listeners() -> void:
	var scene_root = get_tree().current_scene
	if scene_root == null:
		return

	var temp_combat = scene_root if scene_root.name == "TempCombat" else scene_root.find_child("TempCombat", true, false)
	if temp_combat and temp_combat.has_method("discard_selected_cards"):
		discard_requested.connect(Callable(temp_combat, "discard_selected_cards"))

	var manager = scene_root if scene_root.name == "GameManager" else scene_root.find_child("GameManager", true, false)
	if manager and manager.has_method("discard_selected_cards"):
		discard_requested.connect(Callable(manager, "discard_selected_cards"))
