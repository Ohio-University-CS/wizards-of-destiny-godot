...
func clear_enemy_intent() -> void:
	var e1 = enemy_intent_1
	var e2 = enemy_intent_2
	var e3 = enemy_intent_3
	if e1 == null:
		e1 = get_node_or_null("Enemy/EnemyIntent1")
	if e2 == null:
		e2 = get_node_or_null("Enemy/EnemyIntent2")
	if e3 == null:
		e3 = get_node_or_null("Enemy/EnemyIntent3")
	if e1:
		e1.texture = null
	if e2:
		e2.texture = null
...
func update_enemy_intent() -> void:
	...
	var e1 = enemy_intent_1 if enemy_intent_1 != null else get_node_or_null("Enemy/EnemyIntent1")
	var e2 = enemy_intent_2 if enemy_intent_2 != null else get_node_or_null("Enemy/EnemyIntent2")
	var e3 = enemy_intent_3 if enemy_intent_3 != null else get_node_or_null("Enemy/EnemyIntent3")
	if next_move:
		if e1 and next_move.intent_icons.size() > 0:
			e1.texture = next_move.intent_icons[0]
		elif e1:
			e1.texture = null
		if e2:
			if next_move.intent_icons.size() > 1:
				e2.texture = next_move.intent_icons[1]
			else:
				e2.texture = null
		if e3:
			if next_move.intent_icons.size() > 2:
				e3.texture = next_move.intent_icons[2]
			else:
				e3.texture = null
	else:
		if e1:
